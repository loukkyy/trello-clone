...
export default function setup(onDrag, onDragComplete) {
  addGlobalEventListener("mousedown", "[data-draggable]", (e) => {
    ...
    setupDragEvents(selectedItem, itemClone, ghost, offset, onDrag, onDragComplete)
  })
}
...
function setupDragEvents(
  ...
  const mouseMoveFunction = (e) => {
    ...
    onDrag({
      ...
      index: Array.from(dropZone.children).indexOf(selectedItem),
    })

    ghost.style.display = "block"
    if(dropZone.dataset.dropZone) {
      const options = JSON.parse(dropZone.dataset.dropZone)
